			...
			<cfif isUserInRole("admin")>
				...
				<!--
					function openCodeFormatter() {
						var cObj = getGUIObjectInstanceById('div_code_formatter');
						if (cObj != null) {
							cObj.style.display = const_inline_style;
						}
						
						var cObj = getGUIObjectInstanceById('btn_formatCode');
						if (cObj != null) {
							cObj.disabled = true;
						}
					}
					...
					function performCodeFormatter() {
						...
						var a = -1;
						var fmtCode = '';
						var cObj = getGUIObjectInstanceById('ta_code_to_format');
						var dObj = getGUIObjectInstanceById('ta_formated_code');
