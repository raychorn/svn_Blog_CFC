...
<cfparam name="attributes.secure" type="boolean" default="true">
...
<cfset _prefix = "https://">
<cfif (isDebugMode()) OR (FindNoCase(".ssl-docs.com", attributes.action) eq 0) OR (NOT attributes.secure)>
	<cfset _prefix = "http://">
</cfif>

<cfscript>
	attributes.action = ReplaceNoCase(attributes.action, 'http://', '');
	iframeSrcDefault = _prefix & iframeSrcDefault;

	if ( ( (isDebugMode()) OR (Request.bool_isDebugMode) ) AND (thisTag.executionMode is "start") ) {
...
<cfif (thisTag.executionMode is "start")>
	<cfoutput>
		<cfset targetPragma = "">
		<cfif (Len(attributes.target) gt 0)>
			<cfset targetPragma = ' target="#attributes.target#"'>
		</cfif>
		<form name="#attributes.formName#" id="#attributes.formName#" action="#_prefix##attributes.action#" enctype="#attributes.enctype#" method="post" #targetPragma#>
	</cfoutput>
<cfelseif (thisTag.executionMode is "end")>
	<cfoutput>
		...
		<cfset iframe_style = "none">
		<cfif ( ( (isDebugMode()) OR (Request.bool_isDebugMode) ) ) AND 0>
			<cfset iframe_style = "inline">
			<small>iframeSrcDefault = [#iframeSrcDefault#]</small><br>
		</cfif>

		<cfset Register.secureFormAction = iframeSrcDefault>
		<iframe id="iframe_sslInterface_#attributes.formName#" frameborder="1" scrolling="Auto" width="100%" height="400" src="#iframeSrcDefault#/ssl/default.html" style="display: #iframe_style#;"></iframe>
	</cfoutput>
</cfif>
