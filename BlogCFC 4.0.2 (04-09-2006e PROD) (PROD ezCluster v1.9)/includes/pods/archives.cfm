	...
	<cfmodule template="../../tags/podlayout.cfm" containerClass="rightMenuWide" title="#application.resourceBundle.getResource("archivesbysubject")#">
		...
		<cfloop query="Request.cats">
			<cfif entryCount>
				...
				<cfelse>
					<cfset _rssURL = Request.commonCode.clusterizeURLForSessionOnly("http://#CGI.SERVER_NAME##ListSetAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, "/"), "rss.cfm", "/")#") & "&mode=full&mode2=cat&catid=#categoryid#">
					<cfset _catURL = Request.commonCode.clusterizeURLForSessionOnly("http://#CGI.SERVER_NAME##ListSetAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, "/"), "_index.cfm", "/")#") & "&mode=cat&catid=#categoryid#">
				</cfif>
