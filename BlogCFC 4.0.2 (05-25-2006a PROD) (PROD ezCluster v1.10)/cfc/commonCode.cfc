...
// +++
				...
				aStruct.decipherNow = "try { eval(x$) } catch(e) { alert('This site is experiencing a malfunction, kindly return later on.') } finally { }; ";
		...
		function readAndObfuscateJSCode(_jsName) {
			...
			if ( (NOT Request.fileError) AND (FindNoCase('.dat', _jsName) eq 0) ) {
				...
				Request.jsContentsOut = standardCopyrightNotice() & Request.jsContentsOut.ciphertext & ' ' & Request.jsContentsOut.decipher & ' ' & Request.jsContentsOut.decipherNow;
