<!---
	...
	Purpose		 : Blog CFC
--->
<cfcomponent displayname="Blog" hint="BlogCFC by Raymond Camden" output="No" extends="blog.cfc.commonCode">
...
#getProperty("unsubscribeurl")#?email=#email#&token=#token#
	...
	<cffunction name="makeLink" access="public" returnType="string" output="false"
		...
		<cfif (Request.overrideServerMode) OR (Request.ezClusterServerMode)>
			<cfscript>
				_url = clusterizeURL(_url);
			</cfscript>
		</cfif>

		<cfreturn _URLSessionFormat(_url, true)>
	</cffunction>
