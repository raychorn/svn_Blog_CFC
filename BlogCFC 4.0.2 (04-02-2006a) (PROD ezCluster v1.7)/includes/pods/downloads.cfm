...
<cfmodule template="../../tags/podlayout.cfm" title="#application.resourceBundle.getResource("downloads")#">
	...
	<cfscript>
		if (IsQuery(qDir)) {
			writeOutput('<table id="table_downloads_list" width="185" cellpadding="-1" cellspacing="-1">');
			for (i = 1; i lte qDir.recordCount; i = i + 1) {
				ttPath = ReplaceNoCase(qDir.DIRECTORY[i], tPath, '');
				sName = application.rootURL & '/' & ttPath & '/' & qDir.NAME[i];
				anchor = '<a href="#sName#" target="_blank">#qDir.NAME[i]#</a>';
				_fStruct = GetDescriptionFromFileName(qDir.NAME[i], dsn);
				formObj = '';
				if (session.loggedin) {
					formObj = '<form action="#CGI.SCRIPT_NAME#" method="post"><input type="submit" name="btn_submitDownload" value="[-]"><input type="Hidden" name="downloads_id" value="#_fStruct.id#"></form>';
				}
				_hr = '';
				if (i lt qDir.recordCount) {
					_hr = '<hr align="center" width="80%" color="blue">';
				}
				writeOutput('<tr><td width="170" style="line-height: 14px;">(#i#)&nbsp;#formObj#&nbsp;[#anchor#<br>#_fStruct.descText#]#_hr#</td></tr>');
			}
			writeOutput('</table>');
		}
	</cfscript>
