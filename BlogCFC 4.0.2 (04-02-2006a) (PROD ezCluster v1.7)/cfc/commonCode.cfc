	...
	<cfscript>
		...
		function _URLSessionFormat(aURL, isMakingLink) {
			...
			if (IsDefined("Session.urltoken")) {
				...
				for (j = 1; j lte nn; j = j + 1) {
					...
					if (ArrayLen(_ar2) eq 2) {
						Request.commonCode.safely_execSQL('Request.qCheckName', '', "SELECT * FROM Request.qQ WHERE (name = '#_ar2[1]#')");
			...
			for (i = 1; i lte Request.qQ.recordCount; i = i + 1) {
				if (isMakingLink) {
					if ( (UCASE(Request.qQ.name[i]) eq 'CFID') OR (UCASE(Request.qQ.name[i]) eq 'CFTOKEN') OR (UCASE(Request.qQ.name[i]) eq 'jsessionid') ) {
