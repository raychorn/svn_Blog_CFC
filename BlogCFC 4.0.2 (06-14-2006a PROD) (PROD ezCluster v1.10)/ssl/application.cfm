...
<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
	...
	<cfif (UCASE(CGI.REQUEST_METHOD) eq "POST") AND (isDefined("form.username")) and (isDefined("form.password")) and (len(trim(form.username))) and (len(trim(form.password)))>
		...
		<cflog file="#Application.applicationName#_DEBUG" type="Information" text="AA. Request.bool_isValid = [#Request.bool_isValid#], Request.qAuthUser.recordCount = [#Request.qAuthUser.recordCount#]">
		<cfif (Request.bool_isValid)>
			<cfscript>
				Request.commonCode.retireSessions();
			...
			<cflog file="#Application.applicationName#_DEBUG" type="Information" text="A. Request.bool_isValid = [#Request.bool_isValid#], Request.qCheckUsersSession.recordCount = [#Request.qCheckUsersSession.recordCount#] [#_sqlStatement#]">
			<cfif (Request.bool_isValid)>
				<cflogin>
