...
<cfif (IsDefined("FORM.USERNAME")) AND (IsDefined("FORM.YOURNAME")) AND (IsDefined("FORM.PASSWORD")) AND (IsDefined("FORM.CONFIRMPASSWORD"))>
	...
	<cfset _urlValidateLink = Request.commonCode.clusterizeURL('http://#CGI.SERVER_NAME#') & ListSetAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), 'validateUserAccount/#FORM.USERNAME#/', '/')>
...
<cfsavecontent variable="registerForm">
	...
	<cfif (showForm)>
		...
		<form id="form_register_newUser" action="#_url#" method="post" enctype="application/x-www-form-urlencoded">
		...
		<table width="100%">
			...
			<tr>
				<td width="15%"><input type="submit" disabled class="buttonClass" id="button_registerSubmit" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> value="#application.resourceBundle.getResource("register")#" onclick="delayedButtonDisablerByID(this.id, 'form_register_newUser'); return false;"></td>
