...
<cfif not isDefined("attributes.scope") or not isSimpleValue(attributes.scope) or not listFindNoCase("application,session,server,db",attributes.scope)>
	<cfthrow message="scopeCache: The scope attribute must be passed as one of: application, session, server or db.">
</cfif>

<cfset bool_scopeIsDb = (attributes.scope IS "db")>
...
<cfif isDefined("attributes.clearAll")>
	...
	<cfelse>
		<cfscript>
			_sqlStatement = "DELETE FROM ScopeCache WHERE (sessID = #Session.sessID#)";
			Request.commonCode.safely_execSQL('Request.qDropScopeCacheForSession', Request.ClusterDB_DSN, _sqlStatement);
			if (Request.dbError) {
				Request.commonCode.cf_log(Application.applicationname, 'Error', '[#CGI.SCRIPT_NAME#] (ScopeCache.cfm) [#Request.explainErrorText#] [#_sqlStatement#]');
			}
		</cfscript>
	</cfif>
...
<cfif isDefined("attributes.clear") AND (NOT bool_scopeIsDb) AND attributes.clear AND structKeyExists(ptr.scopeCache,attributes.name) AND thisTag.executionMode is "start">
	<cfset cleanup = ptr.scopeCache[attributes.name].dependancies>
	<cfset structDelete(ptr.scopeCache,attributes.name)>
	<cfset dontRun = true>
<cfelseif (isDefined("attributes.clear")) AND (NOT bool_scopeIsDb) AND (attributes.clear) AND (IsDefined("attributes.name")) AND (Len(attributes.name) gt 0) AND (thisTag.executionMode is "start")>
	<cfscript>
		_sqlStatement = "DELETE FROM ScopeCache WHERE (sessID = #Session.sessID#) AND (cacheName = '#attributes.name#')";
		Request.commonCode.safely_execSQL('Request.qDropNamedScopeCacheForSession', Request.ClusterDB_DSN, _sqlStatement);
		if (Request.dbError) {
			Request.commonCode.cf_log(Application.applicationname, 'Error', '[#CGI.SCRIPT_NAME#] (ScopeCache.cfm) [#Request.explainErrorText#] [#_sqlStatement#]');
		}
	</cfscript>
...
<cfif not dontRun>
	<cfif thisTag.executionMode is "start">
		...
		<cfelse>
			<!--- BEGIN: Determine if the timeout in the Db has expired or not... --->
			<cfscript>
				_signal_exit = false;
				
				_sqlStatement = "SELECT id, sessID, cacheName, timeout, cache FROM ScopeCache WHERE (sessID = #Session.sessID#) AND (cacheName = '#Request.commonCode.filterQuotesForSQL(attributes.name)#') AND (GetDate() < timeout)";
				Request.commonCode.safely_execSQL('Request.qValidateScopeCacheForSession', Request.ClusterDB_DSN, _sqlStatement);
				if (Request.dbError) {
					Request.commonCode.cf_log(Application.applicationname, 'Error', '[#CGI.SCRIPT_NAME#] (ScopeCache.cfm) [#Request.explainErrorText#] [#_sqlStatement#]');
				} else if (Request.qValidateScopeCacheForSession.recordCount gt 0) {
					if (NOT isDefined("attributes.r_Data")) {
						writeOutput(Request.qValidateScopeCacheForSession.cache);
					} else {
						caller[attributes.r_Data] = Request.qValidateScopeCacheForSession.cache;
					}
					_signal_exit = true;
				}
			</cfscript>
			...
			<!--- END! Determine if the timeout in the Db has expired or not... --->
		</cfif>
	<cfelse>
		...
		<cfelse>
			<!--- BEGIN: Save the content in the Db... --->
			<cfscript>
				_sqlStatement = "SELECT id FROM ScopeCache WHERE (sessID = #Session.sessID#) AND (cacheName = '#Request.commonCode.filterQuotesForSQL(attributes.name)#')";
				Request.commonCode.safely_execSQL('Request.qCheckScopeCacheForSession', Request.ClusterDB_DSN, _sqlStatement);
				if (Request.dbError) {
					Request.commonCode.cf_log(Application.applicationname, 'Error', '[#CGI.SCRIPT_NAME#] (ScopeCache.cfm) [#Request.explainErrorText#] [#_sqlStatement#]');
				} else {
					_content = '';
					if (NOT isDefined("attributes.data")) {
						_content = thistag.generatedcontent;
					} else {
						_content = attributes.data;
					}
					if (Request.qCheckScopeCacheForSession.recordCount gt 0) { // do update...
						_sqlStatement = "UPDATE ScopeCache SET timeout = #CreateODBCDateTime(ParseDateTime(attributes.timeout))#, cache = '#Request.commonCode.filterQuotesForSQL(_content)#' WHERE (id = #Request.qCheckScopeCacheForSession.ID#)";
					} else { // do insert...
						_sqlStatement = "INSERT INTO ScopeCache (sessID, cacheName, timeout, cache) VALUES (#Session.sessID#,'#Request.commonCode.filterQuotesForSQL(attributes.name)#',#CreateODBCDateTime(ParseDateTime(attributes.timeout))#,'#Request.commonCode.filterQuotesForSQL(_content)#'); SELECT @@IDENTITY as 'id';";
					}

					Request.commonCode.safely_execSQL('Request.qSaveScopeCacheForSession', Request.ClusterDB_DSN, _sqlStatement);
					if (Request.dbError) {
						Request.commonCode.cf_log(Application.applicationname, 'Error', '[#CGI.SCRIPT_NAME#] (ScopeCache.cfm) [#Request.explainErrorText#] [#_sqlStatement#]');
					}

					if (isDefined("attributes.r_Data")) {
						caller[attributes.r_Data] = _content;
					}
				}
			</cfscript>
			<!--- END! Save the content in the Db... --->
		</cfif>
	</cfif>
</cfif>
