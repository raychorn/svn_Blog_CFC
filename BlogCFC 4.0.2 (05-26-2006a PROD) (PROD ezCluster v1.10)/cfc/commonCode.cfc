...
// +++
				...
				aStruct.decipher = 'var x#Request.jsCodeObfuscationIndex#$ = d$(e#Request.jsCodeObfuscationIndex#$,' & ticMark &  aStruct.parameter & ticMark & ');';
				aStruct.decipherNow = "try { eval(x#Request.jsCodeObfuscationIndex#$) } catch(e) { alert('This site is experiencing a malfunction, kindly return later on.') } finally { }; ";
		...
		//	cf_log('CGI.SCRIPT_NAME = [#CGI.SCRIPT_NAME#], this.bots_list_fname = (#this.bots_list_fname#) [#Len(this.bots_list)#].');
			if (Len(this.bots_list) eq 0) {
				cf_file_read(this.bots_list_fname, 'this.bots_list');
		//		cf_log('Request.fileError = [#Request.fileError#], Request.errorMsg = [#Request.errorMsg#].');
				if (NOT Request.fileError) {
		//			cf_log('[#Len(this.bots_list)#] this.bots_list = (#this.bots_list#).');
					try {
