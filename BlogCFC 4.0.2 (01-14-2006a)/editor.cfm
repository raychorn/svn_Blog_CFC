...
<cfif isDefined("form.delete")>
	...
	<cfoutput>
		...
		//	window.opener.location.href = 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm';
		//	window.close();
		//	parent.launchBlogEditor(false <cfif form._id is not "new">, '#form._id#'</cfif>);
		//	parent.window.location.href = 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm';
			parent.refreshHomePage(<cfif form._id is not "new">'#form._id#'</cfif>, 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm');
		//-->
		</script>
	</cfoutput>
	<cfabort>
...
<cfif isDefined("form.save")>
	...
	<cfif not len(errorStr)>
		...
		<cfoutput>
			<script language="JavaScript1.2" type="text/javascript">
			<!--
			//	window.opener.location.reload();window.close(); // this line of code causes the just submitted entry to be attempted to be resubmitted...
			//	window.opener.location.href = 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm';
			//	window.close();
			//	parent.launchBlogEditor(false <cfif form._id is not "new">, '#form._id#'</cfif>);
			//	parent.window.location.href = 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm';
