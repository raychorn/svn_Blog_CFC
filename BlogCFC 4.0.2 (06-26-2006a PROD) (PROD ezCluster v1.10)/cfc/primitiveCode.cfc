...
<!--- +++ --->
	...
	<cffunction name="readSessionFromDb" access="public">
		...
		<cfscript>
			if ( ( (isDebugMode()) OR (isServerLocal()) ) AND (FindNoCase('/newDownloads.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/newCalendar.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/ezAJAXLogo.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/AJAX_functions.cfm', CGI.SCRIPT_NAME) eq 0) ) {
