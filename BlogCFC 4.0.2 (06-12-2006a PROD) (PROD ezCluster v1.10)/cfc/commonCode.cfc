...
// +++
		...
		function performUserRegistration(username, yourname, password) {
			...
			_sqlStatement = "INSERT INTO tblUsers (username, uName, password, uRole, isValid) VALUES ('#Request.commonCode.filterQuotesForSQL(username)#','#Request.commonCode.filterQuotesForSQL(yourname)#','#Request.commonCode.encodedEncryptedString(Request.commonCode.filterQuotesForSQL(password))#','User',0); SELECT @@IDENTITY as 'id';";
			...
			if ( (Request.dbError) AND (NOT Request.isPKviolation) ) {
				...
				_sqlStatement = "SELECT id FROM tblUsers WHERE (username = '#Request.commonCode.filterQuotesForSQL(username)#')";
			...
			if (NOT Request.showForm) {
				Request.commonCode.safely_cfmail(username, 'do-not-respond@contentopia.net', 'User Account Validation EMail from #instance.blogTitle#', registerNotice);
