...
<cfif (session.persistData.loggedin)>
	...
	<cfif isDefined("form.addtrackback")>
		...
		<cfif not len(trim(form.url)) or not Request.commonCode.isURL(form.url)>
...
<cfloop query="trackbacks">
	...
	<cfif session.persistData.loggedin> | <a href="#Request.commonCode.makeLinkToSelf('', true)#&id=#url.id#&delete=#id#">#application.resourceBundle.getResource("delete")#</a></cfif></div>		
	<hr noshade size=1>
	</p>
	</cfoutput>
</cfloop>
...
<cfif (session.persistData.loggedin)>
	...
	<form action="#_url#&id=#url.id#" method="post" enctype="application/x-www-form-urlencoded">
	<table>
	<tr>
		<td><NOBR>Your Blog Name:</NOBR></td>
		<td><input type="text" name="blog_name" value="#form.blog_name#" size="80" maxlength="255"></td>
	</tr>
	<tr>
		<td><NOBR>Your Blog Entry Title:</NOBR></td>
		<td><input type="text" name="title" value="#form.title#" size="80" maxlength="255"></td>
	</tr>
	<tr>
		<td colspan=2>
		Excerpt from your Blog Entry:<br>
		<textarea name="excerpt" cols=100 rows=10>#form.excerpt#</textarea>
		</td>
	</tr>
	<tr>
		<td><NOBR>Your Blog Entry URL:</NOBR></td>
		<td><input type="text" name="url" value="#form.url#" size="80" maxlength="255"></td>
	</tr>
	<cfif (IsDefined("Session.sessID"))><input type="hidden" name="sessID" value="#Session.sessID#"></cfif>
	<tr>
		<td>&nbsp;</td>
		<td><input type="submit" name="addtrackback" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> value="#application.resourceBundle.getResource("post")#"></td>
	</tr>
	</table>
...
<tr>
	<td>Trackback URL:</td>
	<td><input type="text" name="trackbackURL" value="" size="80" maxlength="255"></td>
</tr>
<tr>
	<td>Blog Name:</td>
	<td><input type="text" name="blog_name" value="#application.blog.getProperty("blogTitle")#" size="80" maxlength="255"></td>
</tr>
<tr>
	<td>Your Blog Entry Title:</td>
	<td><input type="text" name="title" value="#article.title#" size="80" maxlength="255"></td>
</tr>
<tr>
	<td colspan=2>
	Excerpt from your Blog Entry:<br>
	<textarea name="excerpt" cols=100 rows=10>#left(article.body,255)#</textarea>
	</td>
</tr>
<tr>
	<td>Your Blog Entry URL:</td>
	<td><input type="text" name="url" value="#application.blog.makeLink(url.id)#" size="80" maxlength="255"></td>
</tr>
<cfif (IsDefined("Session.sessID"))><input type="hidden" name="sessID" value="#Session.sessID#"></cfif>
<tr>
	<td>&nbsp;</td>
