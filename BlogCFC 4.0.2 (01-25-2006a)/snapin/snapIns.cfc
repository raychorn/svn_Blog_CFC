<cfcomponent displayname="snapIns">
	...
	<cfscript>
		...
		function objectForType(objType) {
			...
			if (NOT bool_isError) {
				bool_isError2 = cf_directory('Request.qqDir2', ListDeleteAt(CGI.CF_TEMPLATE_PATH, ListLen(CGI.CF_TEMPLATE_PATH, '\'), '\'), 'commonCode.cfc', true);
				try {
			...
			try {
			   anObj = CreateObject("component", dotPath).init();
				...
				writeOutput('<font color="red"><b>#Request.err_objectFactoryMsg# [dotPath=#dotPath#], [thePath=#thePath#], [bool_isError=#bool_isError#], [bool_isError2=#bool_isError2#], (#Request.explainError(e, true)#)</b></font><hr color="blue" width="80%">' & explainQueryObject(Request.qqDir) & '<hr color="blue" width="80%">' & explainQueryObject(Request.qqDir2));
			}
			return anObj;
		}
