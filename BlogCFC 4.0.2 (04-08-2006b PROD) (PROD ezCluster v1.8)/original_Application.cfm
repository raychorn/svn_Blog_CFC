...
<cfelse>
	<!--- BEGIN: This has to be here because the scope cache thing works against us when running on the Cluster... --->
	<cflock timeout="1" throwontimeout="No" type="EXCLUSIVE" scope="APPLICATION">
		<cfset application.init = false>
	</cflock>
	<!--- END! This has to be here because the scope cache thing works against us when running on the Cluster... --->
</cfif>
...
<cfif 0>
		...
		<cfscript>
			if (IsDefined("application.crossBrowserLibraryJSCode")) {
				StructDelete(application, 'crossBrowserLibraryJSCode');
			}
		</cfscript>
</cfif>		
