<cfcomponent>
	...
	<cffunction name="onRequestStart" access="public">
		...
		<cfscript>
			...
			if (IsDefined("Request.qQ")) {
				_sqlStatement = "SELECT * FROM Request.qQ WHERE (name = 'jsessionid')";
				Request.commonCode.safely_execSQL('Request.qJsessionid', '', _sqlStatement);
			}
			
			_jsessionid = Session.sessionid;
			if (IsDefined("Request.qJsessionid.val")) {
				_jsessionid = Request.qJsessionid.val;
			} else if (IsDefined("URL.JSESSIONID")) {
				_jsessionid = URL.JSESSIONID;
			}
		...
		<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
			<cfscript>
				...
				if ( (0) AND (Request.bool_isDebugUser) AND (ListFindNoCase(doNotDebugThesePages, ListLast(CGI.SCRIPT_NAME, '/'), ',') eq 0) ) {
					if (IsDefined("Request.qQ")) Request.commonCode.cf_dump(Request.qQ, 'A. Request.qQ', false);
					if (IsDefined("Request.qJsessionid")) Request.commonCode.cf_dump(Request.qJsessionid, 'A. Request.qJsessionid', false);
					if (IsDefined("Request.qCheckSession")) Request.commonCode.cf_dump(Request.qCheckSession, 'A. Request.qCheckSession [#ListLast(CGI.SCRIPT_NAME, "/")#] [#Request.ClusterDB_DSN#] [#_sqlStatement#] [#Request.dbError#]', false);
				}
				...
				if ( (0) AND (Request.bool_isDebugUser) AND (ListFindNoCase(doNotDebugThesePages, ListLast(CGI.SCRIPT_NAME, '/'), ',') eq 0) ) {
					Request.commonCode.cf_dump(Session, 'B. Session', false);
				}
