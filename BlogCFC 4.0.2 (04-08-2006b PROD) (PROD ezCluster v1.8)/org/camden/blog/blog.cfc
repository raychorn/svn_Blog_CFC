		...
		<cfelse>
			<cfscript>
				...
				if ( (NOT Request.dbError) AND (IsDefined("Request.qGetValidBlogUsers")) AND (IsQuery(Request.qGetValidBlogUsers)) ) {
					...
					_sqlStatement = "SELECT id, username, uName, password, uRole, isValid FROM Request.qGetValidBlogUsers WHERE (username = '#Request.commonCode.filterQuotesForSQL(arguments.username)#') AND (isValid = 1) AND (password = '#Request.commonCode.filterQuotesForSQL(arguments.password)#')";
