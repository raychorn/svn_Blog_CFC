	...
	<cfscript>
		...
		if (bool_RuntimeLicenseStatus) {
			if (FindNoCase(CGI.SERVER_NAME, CGI.HTTP_REFERER) gt 0) {
				if ( (IsDefined("Request.qryStruct.sessid")) AND (IsValid('UUID', Request.qryStruct.sessid)) AND (IsDefined("Session.persistData.loggedin")) AND (Session.persistData.loggedin) ) {
