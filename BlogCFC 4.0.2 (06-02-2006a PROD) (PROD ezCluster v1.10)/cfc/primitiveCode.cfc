	...
	<cffunction name="readSessionFromDb" access="public">
		...
		<cfscript>
			if ( ( (isDebugMode()) OR (Request.commonCode.isServerLocal()) ) AND (FindNoCase('/newDownloads.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/newCalendar.cfm', CGI.SCRIPT_NAME) eq 0) ) {
				writeOutput('<small style="font-size: 10px; color: blue;"><b>(#_sessID#) (#_referrer#) (#_serverName#) (#CGI.SCRIPT_NAME#) URL.sessID = [#bool_IsDefined_URL_sessID#], FORM.sessID = [#bool_IsDefined_FORM_sessID#], Client.sessID = [#bool_IsDefined_Client_sessID#], Session.sessID = [#bool_IsDefined_Session_sessID#]</b></small><br>');
			}
		</cfscript>
