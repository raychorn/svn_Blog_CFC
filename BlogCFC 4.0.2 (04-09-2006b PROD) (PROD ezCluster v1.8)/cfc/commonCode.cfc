<cfcomponent displayname="commonCode" output="No" extends="primitiveCode">
	...
	<cfscript>
		...
		function _URLSessionFormat(aURL, isMakingLink) {
			...
			// +++
			queryStr = _dropAllInstancesLike('reinit=', queryStr, '&', 0);
			queryStr = dropAllNonUniqueInstancesExceptLast(queryStr, '&'); // keeps only the last unique instance of every URL parm the system might manipulate...
			queryStr = _dropAllInstancesLike('CFID=', queryStr, '&', 0);
			queryStr = _dropAllInstancesLike('CFTOKEN=', queryStr, '&', 0);
			queryStr = _dropAllInstancesLike('jsessionid=', queryStr, '&', 0);
			queryStr = _dropAllInstancesLike('sessID=', queryStr, '&', 0);
//	writeOutput('queryStr = [#queryStr#]<br>');
			// +++
			...
			queryStr = '';
			queryStr = ListAppend(queryStr, 'sessID=#Session.sessID#', '&');
			for (i = 1; i lte Request.qQ.recordCount; i = i + 1) {
				if ( (NOT isMakingLink) OR ( (UCASE(Request.qQ.name[i]) neq 'CFID') AND (UCASE(Request.qQ.name[i]) neq 'CFTOKEN') AND (UCASE(Request.qQ.name[i]) neq 'jsessionid') ) ) {
		...
		function makeLinkToSelfBase(aURL) {
			...
			return _URLSessionFormat(clusterizeURL('http://#CGI.SERVER_NAME#/#myPrefix#/#aURL#'), false);
		}
		...
		function setScopeCacheToClearOnAllServers(bool_clearScopeCache) {
			var iServerNum = -1;
			var nServerName = -1;
			var arServerName = -1;
			var newServerName = -1;
			var arNewServerName = -1;
			if (NOT IsBoolean(bool_clearScopeCache)) {
				bool_clearScopeCache = false;
			}
			try {
				if (Request.isRunningOnCluster) {
					arServerName = ListToArray(CGI.SERVER_NAME, '.');
					for (iServerNum = 1; iServerNum lte 2; iServerNum = iServerNum + 1) {
						arNewServerName = ArrayNew(1);
						arNewServerName[1] = arServerName[1] & '.#iServerNum#';
						nServerName = ArrayLen(arServerName);
						for (i = 2; i lte nServerName; i = i + 1) {
							arNewServerName[i] = arServerName[i];
						}
						newServerName = ArrayToList(arNewServerName, '.');
						Session.clearedScopeCacheOnServerByName[newServerName].clearScopeCache = bool_clearScopeCache;
					}
				} else {
					Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache = bool_clearScopeCache;
				}
			} catch (Any e) {
			}
		}
