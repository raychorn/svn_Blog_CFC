...
<cftry>
	<cfif (IsDefined("Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache")) AND (Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache)>
		<cfmodule template="tags/scopecache.cfm" scope="application" clearall="true">
		<cfset Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache = false>
	</cfif>
...
<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
	<cfif (UCASE(CGI.REQUEST_METHOD) eq "POST") AND (isDefined("form.username")) and (isDefined("form.password")) and (len(trim(form.username))) and (len(trim(form.password)))>
		...
		<cfif (IsDefined("Request.qAuthUser")) AND (Request.bool_isDebugMode) AND 0>
			<cfdump var="#Request.qAuthUser#" label="Request.qAuthUser (#bool_isValid#)" expand="No">
		</cfif>
		<cfif (bool_isValid)>
			<cflogin>
