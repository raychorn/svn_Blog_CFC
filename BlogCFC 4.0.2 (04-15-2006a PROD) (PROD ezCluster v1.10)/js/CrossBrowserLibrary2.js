...
function _validateUserAccountName(oObj, btnObj) {
	if (!!oObj) {
		...
		if (!!btnObj) btnObj.disabled = ( (_f == -1) || (_f2a != -1) || (_f2b != -1) || (_f2c != -1) ); 
		if ( (_f == -1) || (_f2a != -1) || (_f2b != -1) || (_f2c != -1) ) {
			oObj.style.border = 'thin solid red';
			oObj.title = 'Your User Name is NOT acceptable because ' + ((_f == -1) ? '"your domain name uses an invalid TLD" ' : '') + (( (_f2a != -1) || (_f2b != -1) || (_f2c != -1) ) ? '"your domain name is for an anonymous email service which is a EULA Violation" ' : '') + '.';
		} else {
