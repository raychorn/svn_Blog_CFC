...
function showAJAXBegins(sMsg, sImage, imageHeight) {
	...
	if (!!cObj) {
		...
		resp += '<link rel=stylesheet type="text/css" href="' + document.location.href + ((document.location.href.charAt(document.location.href.length - 1) != '/') ? '/' : '') + 'AJAX/ajaxStyles.css">';
		resp += '</head>';
		resp += '<body>';
		resp += ajaxBegins();
		resp += '<b>' + ((!!sMsg) ? sMsg : const_loading_data_message_symbol) + '</b>';
		resp += '&nbsp;&nbsp;<img height="' + imageHeight + '" src="' + ((!!sImage) ? sImage : const_jsapi_loading_image) + '" border="0">';
		resp += ajaxEnds();
		resp += '</body></html>';
