		...
		<cfmodule template="tags/secureForm.cfm" formName="form_register_newUser" action="#_url#">
			...
			<table width="100%">
				...
				<tr>
					...
					<td width="15%">
						<cfset _jsCode = "if (!!performSubmitRegisterForm) { performSubmitRegisterForm(this, 'iframe_sslInterface_form_register_newUser', '#_url#'); };">
						<cfif (isUsingAJAX)>
							<cfset _jsCode = "this.disabled = true; if (!!parent.performSubmitRegisterForm) { var iObj1 = $('register_input_username'); var iObj2 = $('register_input_yourname'); var iObj3 = $('register_input_password'); var iObj4 = $('register_input_confirmpassword'); parent.performSubmitRegisterForm(iObj1.value, iObj2.value, iObj3.value, iObj4.value); } else { alert('Cannot submit this dialog due to a technical issue that will be resolved soon.'); };">
						</cfif>
						<input type="submit" disabled class="buttonClass" id="button_registerSubmit" value="#application.resourceBundle.getResource("register")#" onclick="#_jsCode# return false;">
					</td>
					...
					<td width="*">
						<cfset _jsCode = "var _url = '#_cancelURL#'; this.disabled = true; window.location.href = _url;">
						<cfif (isUsingAJAX)>
