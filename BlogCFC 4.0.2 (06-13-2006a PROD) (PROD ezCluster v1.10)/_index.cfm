...
<cfparam name="url.entryFrom" type="string" default="">
...
<cfscript>
	filteredQS = CGI.QUERY_STRING;
	for (i = 1; i lte ListLen(filteredQS, "&"); ) {
		item = ListGetAt(filteredQS, i, "&");
		if (FindNoCase('sessid=', item) gt 0) {
			filteredQS = ListDeleteAt(filteredQS, i, "&");
		} else {
			i = i + 1;
		}
	}
//	writeOutput('filteredQS = [#filteredQS#]<br>');
</cfscript>

<cfif (Len(url.entryFrom) eq 0) AND (ListLen(filteredQS, "&") eq 0)>
	<cfsetting showdebugoutput="No">
	<cfset ezAJAXProductName = '<NOBR>ezAJAX<sup style="font-size: 9px; font-weight: bold;">(tm)</sup></NOBR>'>
	<cfoutput>
		<table width="80%" cellpadding="2" cellspacing="1" align="center">
			...
			<tr>
				<td width="50%" valign="top" align="center">
					<button id="btn_nonAJAXBlog" class="buttonClass" onclick="var hRef = window.location.href; hRef += ((hRef.indexOf('?') > -1) ? '&' : '?') + 'entryFrom=1'; window.location.href = hRef; /* alert(this.id + '\n' + hRef); */ return false;">Enter the Non-AJAX Blog</button>
				</td>
				<td width="50%" valign="top" align="center">
					<button id="btn_ezAJAXBlog" class="buttonClass" onclick="var hRef = window.location.href; var ar = hRef.split('?'); var ar2 = ar[0].split('/'); ar2[ar2.length - 1] = 'AJAX-Framework/index.cfm'; ar[0] = ar2.join('/'); hRef = ar.join('?'); window.location.href = hRef; /* alert(this.id + '\n' + hRef); */ return false;">Enter the ezAJAX Blog</button>
				</td>
			</tr>
		</table>
	</cfoutput>
