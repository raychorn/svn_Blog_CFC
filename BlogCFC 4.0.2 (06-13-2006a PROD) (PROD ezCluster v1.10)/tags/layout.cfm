...
<cfparam name="suppressDebug" type="integer" default="0">
...
<cfparam name="attributes.suppressLayout" type="boolean" default="#(FindNoCase('/AJAX-Framework/', CGI.HTTP_REFERER) gt 0)#">
													...
													<td colspan="3">
														<cfif (IsDefined("Session.persistData.loggedin")) AND (NOT Session.persistData.loggedin)>
															<cfif (FindNoCase("/login.cfm", CGI.SCRIPT_NAME) eq 0) AND (FindNoCase("/register.cfm", CGI.SCRIPT_NAME) eq 0)>
																...
																<cfset _urlImg = Request.commonCode.clusterizeURL('http://#CGI.SERVER_NAME#') & '/' & ListFirst(CGI.SCRIPT_NAME, '/') & '/images/registerButton.gif'>
																<cfif 1>
																...
																<cfelse>
																	<button name="btn_registerButton" id="btn_registerButton" type="submit" class="registerButtonClass2" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> onclick="this.disabled = true; window.location.href = '#_url#'; return false;"><img src="#_urlImg#" width="110" height="32" border="0"></button>
																</cfif>
															</cfif>
														</cfif>
														<cfif (FindNoCase("/login.cfm", CGI.SCRIPT_NAME) eq 0) AND (FindNoCase("/register.cfm", CGI.SCRIPT_NAME) eq 0)>
...
<script language="JavaScript1.2" type="text/javascript">
	...
	tid_ajax_updateBlogStatsDateTime = setInterval("ajax_updateBlogStatsDateTime()", 1000 * 30);
</script>
