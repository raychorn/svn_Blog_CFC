...
<cfelse>
	<cftry>
		...
		<cfif entry.allowcomments is false>
			<cfset closeMe = true>
		</cfif>
		<cfcatch>
			<cfset closeMe = true>
	...
	<cfoutput>
		...
		<!--
			function closeCommentsAndRefreshHomePage(id) {
				...
				var url = "http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm" + '?uuid=' + parent.uuid() + "##" + id;
...
<cfoutput>
<div class="date">#application.resourceBundle.getResource("comments")#: #entry.title#</div>
<div class="body">
</cfoutput>
...
<cfif (IsDefined("form.id"))>
	...
	<!--
		...
		var url = "http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm" + '?uuid=' + parent.uuid() + "###form.id#";
		parent.window.location.href = url;
	//-->
	</script>
	</cfoutput>
