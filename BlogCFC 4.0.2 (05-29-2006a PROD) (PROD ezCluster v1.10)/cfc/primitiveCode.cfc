...
<!--- +++ --->
	...
	<cffunction name="readSessionFromDb" access="public">
		...
		<cfscript>
			if ( (isDebugMode()) AND (FindNoCase('/newDownloads.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/newCalendar.cfm', CGI.SCRIPT_NAME) eq 0) ) {
		...
		<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
			...
			//	writeOutput(Request.commonCode.cf_dump(Request.qCheckSession, 'Request.qCheckSession [#_sqlStatement#]', false));
				...
				if (Request.dbError) {
					...
					if (Request.dbError) {
						...
						for (i = 1; i lte Request.qChkForRetiredUserSessions.recordCount; i = i + 1) {
							_sqlStatement = "SELECT id FROM Sessions WHERE (id = #Request.qChkForRetiredUserSessions.sessID[i]#)";
						...
						_sqlStatement = "DELETE FROM tblUsersSession WHERE (sessID IN (#listOfAllSessionIDs#))";
