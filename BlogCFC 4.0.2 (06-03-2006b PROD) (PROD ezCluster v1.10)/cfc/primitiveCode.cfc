...
<!--- +++ --->
	...
	<cffunction name="readSessionFromDb" access="public">
		<cfscript>
			...
			_sessID = -1;
			if (UCASE(_referrer) eq UCASE(_serverName)) { // this inhibits an outside usage of the session ID from being abused...
		...
		<cfscript>
			if ( ( (isDebugMode()) OR (Request.commonCode.isServerLocal()) ) AND (FindNoCase('/newDownloads.cfm', CGI.SCRIPT_NAME) eq 0) AND (FindNoCase('/newCalendar.cfm', CGI.SCRIPT_NAME) eq 0) ) {
				...
				writeOutput('<small style="font-size: 9px; color: blue;"><b>(#_urlAJAXBlog#) (#_referrer#) (#_serverName#) (#CGI.SCRIPT_NAME#) URL.sessID = [#bool_IsDefined_URL_sessID#], FORM.sessID = [#bool_IsDefined_FORM_sessID#], Client.sessID = [#bool_IsDefined_Client_sessID#], Session.sessID = [#bool_IsDefined_Session_sessID#]</b></small><br>');
			}
		</cfscript>
			...
			<cfscript>
				...
				if (NOT Request.dbError) {
						...
						if (NOT Request.dbError) {
							...
							if (NOT Request.dbError) {
								if (Request.qCheckApplication.recordCount gt 0) {
									...
									_sqlStatement = "INSERT INTO Sessions (appID, sessionUUID, sessionDt, lastAccessDt, _wddx) VALUES (#Request.qCheckApplication.id#,'#_sessionUUID#',GetDate(),GetDate(),'#Request.commonCode.filterQuotesForSQL(_wddxPacket)#'); SELECT @@IDENTITY as 'id';";
					...
					if ( (isDebugMode()) OR (Request.commonCode.isServerLocal()) ) {
						...
						writeOutput(Request.commonCode.cf_dump(Request, 'Request', false));
					}
					cf_log('[#Request.explainErrorText#] [#_sqlStatement#]');
				}
			</cfscript>
	...
	<cffunction name="retireSessions" access="public">
		...
		<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
			<cfscript>
				...
				Request.commonCode.safely_execSQL('Request.qGetRetirableSessions2', application.blog.instance.dsn, _sqlStatement);
				if (Request.dbError) {
				...
				if (ArrayLen(ar_retireableSessions) gt 0) {
					...
					Request.commonCode.safely_execSQL('Request.qChkForRetiredUserSessions', application.blog.instance.dsn, _sqlStatement);
					if (Request.dbError) {
						...
						for (i = 1; i lte Request.qChkForRetiredUserSessions.recordCount; i = i + 1) {
							...
							Request.commonCode.safely_execSQL('Request.qGetRetirableSessions2a', Request.ClusterDB_DSN, _sqlStatement);
							if (Request.dbError) {
						...
						if (ListLen(listOfAllSessionIDs, ',') gt 0) {
							...
							Request.commonCode.safely_execSQL('Request.qRetireDeadUserSessions', application.blog.instance.dsn, _sqlStatement);
							if (Request.dbError) {
		...
		<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
			<cfscript>
				if ( (IsDefined("Session.sessID")) AND (Session.sessID gt 0) ) {
					...
					_wddxPacket = Request.commonCode.cf_wddx_CFML2WDDX(Session.persistData);
					_sqlStatement = "UPDATE Sessions SET _wddx = '#Request.commonCode.filterQuotesForSQL(_wddxPacket)#', lastAccessDt = GetDate() WHERE (sessionUUID = '#Session.sessID#')";
				...
				_sqlStatement = "INSERT INTO tblUsersActivities (uid, sessID, isloggedin, today, script_name, http_referrer, query_string) VALUES (#_uid#, '#Session.sessID#', #_isloggedin#, GetDate(),'#Request.commonCode.filterQuotesForSQL(CGI.SCRIPT_NAME)#','#Request.commonCode.filterQuotesForSQL(CGI.HTTP_REFERER)#','#Left(Request.commonCode.filterQuotesForSQL(URLDecode(CGI.QUERY_STRING)), 512)#'); SELECT @@IDENTITY as 'id';";
