...
<cfmodule template="tags/layout.cfm">
	...
	<cfoutput query="articles" startrow="#url.startrow#" maxrows="#maxEntries#">
		...
		<h1><a name="#id#" href="#application.blog.makeLink(id)#" target="_top">#title#</a></h1>
		...
		<cfsavecontent variable="_html_">
			...
			<cfloop index="x" from=1 to="#listLen(categoryNames)#">
				...
				<cfset _url_ = Request.commonCode._URLSessionFormat('#application.rooturl#/index.cfm?mode=cat&catid=#listGetAt(categoryIDs,x)#', false)>
				<cfset Request.debuggerOn = false>
	...
	<cfif session.loggedin>| <a href="" onclick="launchBlogEditor(true, '#id#'); return false;">#application.resourceBundle.getResource("edit")#</a></cfif>
		</div>
