...
//	writeOutput('queryStr = [#queryStr#]<br>');
			...
			queryStr = '';
			if (IsDefined("Session.sessID")) queryStr = ListAppend(queryStr, 'sessID=#Session.sessID#', '&');
		...
		function clusterizeURLForSessionOnly(_url) {
			...
			if (IsDefined("Session.sessID")) newURL = '?sessID=#Session.sessID#';
			return newURL;
		}
		
		function _clusterizeURLForSessionOnly(_url) {
			var newURL = _url;
			if (IsDefined("Session.sessID")) newURL = newURL & '?sessID=#Session.sessID#';
			return newURL;
		}
		...
		function isUserAdmin() {
			return ( (IsDefined("Session.persistData.qauthuser.UROLE")) AND (UCASE(Session.persistData.qauthuser.UROLE) eq 'ADMIN') );
		}

		function isUserNormalUser() {
			return ( (IsDefined("Session.persistData.qauthuser.UROLE")) AND (UCASE(Session.persistData.qauthuser.UROLE) eq 'USER') );
		}
