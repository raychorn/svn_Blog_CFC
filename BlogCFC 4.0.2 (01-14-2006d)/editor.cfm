...
<cfif isDefined("form.delete")>
	...
	<cfoutput>
		...
		<!--
			if (!!parent.refreshHomePage) parent.refreshHomePage(<cfif form._id is not "new">'#form._id#'<cfelse>''</cfif>, 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm###form._id#');
		//-->
		</script>
	</cfoutput>
	<cfabort>
...
<cfif isDefined("form.save")>
	...
	<cfif not len(errorStr)>
		...
		<cfoutput>
			<script language="JavaScript1.2" type="text/javascript">
			<!--
				if (!!parent.refreshHomePage) parent.refreshHomePage(<cfif form._id is not "new">'#form._id#'</cfif>, 'http://#CGI.SERVER_NAME##ListDeleteAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '/')#/index.cfm###form._id#');
			//-->
			</script>
		</cfoutput>
		<cfabort>
