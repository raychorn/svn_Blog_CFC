...
<cfif isDefined("form.addcomment")>
	...
	<cfif not len(form.name) AND (NOT IsDefined("Session.qauthuser.UNAME"))>
		<cfset errorStr = errorStr & application.resourceBundle.getResource("mustincludename") & "<br>">
	<cfelseif (IsDefined("Session.qauthuser.UNAME"))>
		<cfset form.name = Session.qauthuser.UNAME>
	</cfif>
	<cfif (not len(form.email) or not Request.commonCode.isEmail(form.email)) AND (NOT IsDefined("Session.qauthuser.USERNAME"))>
		<cfset errorStr = errorStr & application.resourceBundle.getResource("mustincludeemail") & "<br>">
	<cfelseif (IsDefined("Session.qauthuser.USERNAME"))>
		<cfset form.email = Session.qauthuser.USERNAME>
	</cfif>
...
<head>
	...
	<!--
		function closeCommentsAndRefreshHomePage(id) {
			if (!!parent.launchComment) parent.launchComment(id, false); // , '#_url#'
		}
	//-->
	</script>
...
<cfif (session.loggedin) AND (NOT CGI.REQUEST_METHOD IS "POST")>
	...
	<tr>
		<td>&nbsp;</td>
		<td>
		<input type="submit" id="button_commentsSubmit" name="addcomment" value="#application.resourceBundle.getResource("post")#">&nbsp;&nbsp;
...
<cfelseif (NOT IsDefined("session.loggedin")) OR (NOT session.loggedin)>
...
<cfelseif (CGI.REQUEST_METHOD IS "POST")>
	<cfset _url = Request.commonCode._URLSessionFormat(Request.commonCode.makeLinkToSelf("_index.cfm", false), true)>
	<a href="" onclick="if (!!parent.launchComment) parent.launchComment(id, false, '#_url#'); return false;">Click Here to Close the Add Comments Panel and Continue...</a>
</cfif>
...
<cfif 0>
	<cfif (IsDefined("form.id")) AND (CGI.REQUEST_METHOD IS "POST")>
		...
		<script language="JavaScript1.2" type="text/javascript">
		<!--
			if (!!parent.launchComment) parent.launchComment('#_ID#', false, '#_url#');
		//-->
		</script>
		<cfabort>
	</cfif>
