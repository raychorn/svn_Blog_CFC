<cfcomponent>
		...
		function onError(Exception, EventName) {
			...
			if ( (CGI.REMOTE_ADDR eq '127.0.0.1') OR (Find('192.168.', CGI.REMOTE_ADDR) gt 0) ) {
				if (NOT ( (EventName IS "onSessionEnd") OR (EventName IS "onApplicationEnd") ) ) {
					...
					if (UCASE(CGI.SERVER_NAME) eq UCASE('laptop.halsmalltalker.com')) {
	...
	<cffunction name="onRequestStart" access="public">
		...
		<cfscript>
			...
			if (IsDefined("Session.sessID")) {
				...
				_sessID = URL.sessID;
			} else if (IsDefined("FORM.sessID")) {
				_sessID = FORM.sessID;
			} else if (IsDefined("Client.sessID")) {
				_sessID = Client.sessID;
			}
			
			doNotDebugThesePages = 'agooglead.cfm,cfcontent_img.cfm,cfcontent_js.cfm';
		</cfscript>

		<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
			<cfscript>
				_sqlStatement = "SELECT Applications.id as appID, Applications.AppName, Sessions.id AS sessID, Sessions.jsessionid, Sessions.sessionDt, Sessions.lastAccessDt, Sessions._wddx FROM Applications INNER JOIN Sessions ON Applications.id = Sessions.appID WHERE (Sessions.id = '#_sessID#')";
						...
						if (NOT Request.dbError) {
							...
							if (NOT Request.dbError) {
								if (Request.qCheckApplication.recordCount gt 0) {
									...
									_sqlStatement = "INSERT INTO Sessions (appID, jsessionid, sessionDt, lastAccessDt, _wddx) VALUES (#Request.qCheckApplication.id#,'-1',GetDate(),GetDate(),'#Request.commonCode.filterQuotesForSQL(_wddxPacket)#'); SELECT @@IDENTITY as 'id';";
	...
	<cffunction name="onRequestEnd" access="public">
		...
		<cfif (FindNoCase('aGoogleAd.cfm', _targetPage) eq 0)>
			<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
				<cfscript>
					if ( (IsDefined("Session.sessID")) AND (Session.sessID gt 0) ) {
						_wddxPacket = Request.commonCode.cf_wddx_CFML2WDDX(Session);
						...
						if (Request.dbError) {
							...
							if (Request.bool_isDebugMode) {
								writeOutput('<textarea cols="90" rows="10" readonly>#_wddxPacket#</textarea><br>');
								writeOutput(Request.commonCode.cf_dump(Session, 'Session Scope (onRequestEnd)', false));
							}
						}
					}
			...
			<cfscript>
				if ((Request.bool_isDebugMode) AND 0) {
					writeOutput('CGI.SERVER_NAME = [#CGI.SERVER_NAME#], Session.sessID = [#Session.sessID#]<br>');
				}
			</cfscript>
		</cfif>
