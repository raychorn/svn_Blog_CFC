...
<cfsavecontent variable="_html_">
	...
	function validatePassword(s, sOther) {
		...
		if ( (bool_shallSubmitBtnBeDisabled == false) || (bool_passwordsMatches == false) ) {
			...
			disableWidgetByID('button_registerSubmit', false);
			return false;
		}
		return true;
	}
...
<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="APPLICATION">
	<cfif (NOT IsDefined("application.crossBrowserLibraryJSCode")) OR (Len(application.crossBrowserLibraryJSCode) eq 0)>
		<cfscript>
		//	_html_ = Request.commonCode.jsMinifier(Request.commonCode.obfuscateJScode(_html_));
		//	_html_ = Request.commonCode.obfuscateJScode(_html_);
