...
<cfif (session.loginFailure lte 3)>
	<cfsavecontent variable="loginForm">
		...
		<table width="100%">
			...
			<tr>
				<td><input type="submit" class="buttonClass" id="button_loginSubmit" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> value="#application.resourceBundle.getResource("login")#" onclick="delayedButtonDisablerByID(this.id, 'form_login_returningUser'); return false;"></td>
		...
		<cfif ( (IsDefined("URL.loginFailure")) AND (URL.loginFailure gt 0) ) OR ( (IsDefined("Session.loginfailure")) AND (Session.loginfailure gt 0) )>
			...
			<form id="form_login_forgotPassword" action="#_url#" method="post" enctype="application/x-www-form-urlencoded">
				<table width="100%">
					...
					<cfif (IsDefined("Session.sessID"))><input type="hidden" name="sessID" value="#Session.sessID#"></cfif>
					<tr>
						<td><input type="submit" class="buttonClass" id="button_forgotPwdSubmit" value="#application.resourceBundle.getResource("forgotpassword")#" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> onclick="delayedButtonDisablerByID(this.id, 'form_login_forgotPassword'); return false;"></td>
						<td><input type="Button" class="buttonClass" value="#application.resourceBundle.getResource("cancel")#" onclick="var _url = '#_urlHomePage#'; this.disabled = true; window.location.href = _url; return false;"></td>
					</tr>
				</table>
			</form>
