	...
	<cffunction name="onRequestStart" access="public">
			...
			<cfscript>
				...
				if (NOT Request.dbError) {
					if (Request.qCheckSession.recordCount eq 0) {	// create the session record...
								...
								if (Request.qCheckApplication.recordCount gt 0) {
									...
									if (NOT Request.dbError) {
										if (Request.qAddSession.recordCount gt 0) {
											...
											Session.clearedScopeCacheOnServerByName = StructNew(); // allows the actual ScopeeCache clear function to be executed only for a specific server at a time but only when the Session has flagged this action as being necessary...
											Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME] = StructNew(); // setup the specific scope cache flags per server...
											Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache = false;
										}
									} else {
										Request.commonCode.cf_log(Application.applicationname, 'Error', '[#Request.explainErrorText#] [#_sqlStatement#]');
									}
						...
						Session.blogname = 'Default'; // this allows the correct blogname to be known across all servers...
						if (NOT IsDefined("Session.clearedScopeCacheOnServerByName")) {
							Session.clearedScopeCacheOnServerByName = StructNew(); // allows the actual ScopeeCache clear function to be executed only for a specific server at a time but only when the Session has flagged this action as being necessary...
						}
						try {
							Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME] = StructNew(); // setup the specific scope cache flags per server...
		...
		<cfif isDefined("url.logout") and Request.commonCode.isLoggedIn()>
			...
			<cfscript>
				...
				session.loginFailure = 0;
				Request.commonCode.setScopeCacheToClearOnAllServers();
			</cfscript>
			<cflogout>
		</cfif>
