<cfcomponent displayname="commonCode" output="No" extends="primitiveCode">
	...
	if ( (IsDefined("Request.debuggerOn")) AND (Request.debuggerOn) ) writeOutput('<b> #aURL#?#queryStr#</b>');
		...
		function setScopeCacheToClearOnAllServers() {
			var iServerNum = -1;
			var nServerName = -1;
			var arServerName = -1;
			var newServerName = -1;
			...
			try {
				Session.clearedScopeCacheOnServerByName[CGI.SERVER_NAME].clearScopeCache = true;
				...
				if (Request.isRunningOnCluster) {
					...
					for (iServerNum = 1; iServerNum lte 2; iServerNum = iServerNum + 1) {
						...
						Session.clearedScopeCacheOnServerByName[newServerName].clearScopeCache = true;
					}
				}
			} catch (Any e) {
			}
