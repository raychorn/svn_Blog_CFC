<cfcomponent>
		...
		function onError(Exception, EventName) {
			...
			if (NOT ( (EventName IS "onSessionEnd") OR (EventName IS "onApplicationEnd") ) ) {
				if ( (CGI.REMOTE_ADDR eq '127.0.0.1') OR (Find('192.168.', CGI.REMOTE_ADDR) gt 0) ) {
					...
					writeOutput('<h3 align="center" style="color: red;">Something went wrong.  The problem will be fixed a.s.a.p.</h3>');
				}
			}
		}
	</cfscript>
	...
	<cffunction name="onRequestStart" access="public">
		...
		<cfscript>
			...
			Request.doNotSessionThesePages = 'agooglead.cfm,cfcontent_img.cfm,cfcontent_js.cfm';
			...
			<cflock timeout="10" throwontimeout="No" type="EXCLUSIVE" scope="SESSION">
				<cfscript>
					if ( (IsDefined("Session.sessID")) AND (Session.sessID gt 0) ) {
						...
						try {
							ptr = StructGet(Session.persistData);
							if (StructKeyExists(ptr, 'urltoken')) StructDelete(ptr, 'urltoken');
						} catch (Any e) {
						}
					...
					_sqlStatement = "INSERT INTO tblUsersActivities (sessID, isloggedin, script_name, http_referrer, query_string) VALUES (#Session.sessID#, #_isloggedin#, ,'#Request.commonCode.filterQuotesForSQL(CGI.SCRIPT_NAME)#','#Request.commonCode.filterQuotesForSQL(CGI.HTTP_REFERER)#','#Request.commonCode.filterQuotesForSQL(CGI.QUERY_STRING)#'); SELECT @@IDENTITY as 'id';";
