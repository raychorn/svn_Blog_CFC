...
<cfset bool_forceDelete = false>
<cfif (IsDefined("url.delete"))>
	<cfset form.delete = url.delete>
	<cfset bool_forceDelete = true>
</cfif>
...
<cfif isDefined("form.delete")>
	...
	<cfmodule template="tags/scopecache.cfm" cachename="pod_archives" scope="db" clear="true">		
	<cfmodule template="tags/scopecache.cfm" cachename="pod_recent" scope="db" clear="true">		
	<cfoutput>
		...
		<cfif (NOT bool_forceDelete)>
			<script language="JavaScript1.2" type="text/javascript">
			<!--
				if (!!parent.refreshHomePage) {
					id = <cfif form._id is not "new">'#form._id#'<cfelse>''</cfif>;
					parent.refreshHomePage(id, '#_mainURL#');
				}
			//-->
			</script>
		<cfelse>
			<cflocation url="#_mainURL#" addtoken="No">
		</cfif>
	</cfoutput>
	<cfabort>
</cfif>
...
<cfif (IsDefined("form.posted")) AND lsIsDate(form.posted)>
...
<cfset _url = 'http://#CGI.SERVER_NAME##CGI.SCRIPT_NAME#'>
<form action="#_url#?id=#url.id#" method="post" enctype="multipart/form-data">
<table width="100%" border=0>
	...
	<tr>
		<td>&nbsp;</td>
		<td><input type="submit" name="save" <cfif isDebugMode()>title="_url = [#_url#]"</cfif> value="#application.resourceBundle.getResource("save")#">
