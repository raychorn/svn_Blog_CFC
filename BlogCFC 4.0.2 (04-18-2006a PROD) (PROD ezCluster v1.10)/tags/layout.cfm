										...
										<td width="15%" align="center" valign="top">
											<cfif (IsDefined("Session.persistData.loggedin")) AND (NOT Request.commonCode.isUserPremium())>
												<cfif (FindNoCase("/login.cfm", CGI.SCRIPT_NAME) eq 0) AND (FindNoCase("/register.cfm", CGI.SCRIPT_NAME) eq 0)>
													...
													<button name="btn_premiumButton" disabled id="btn_premiumButton" type="submit" class="registerButtonClass" title="Premium Services are under development and will be released soon." onclick="this.disabled = true; window.location.href = '#_url#'; return false;">[Premium]</button>
												</cfif>
											</cfif>
										</td>
