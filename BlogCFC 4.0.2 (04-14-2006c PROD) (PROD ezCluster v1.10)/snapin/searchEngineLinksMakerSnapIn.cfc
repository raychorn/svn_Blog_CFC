			...
			<cfoutput>
				<cfif session.persistData.loggedin>
					<td align="center" valign="bottom">
						<cfset _url = Request.commonCode._clusterizeURLForSessionOnly(Request.commonCode.clusterizeURL('http://#CGI.SERVER_NAME#') & '/' & ListFirst(CGI.SCRIPT_NAME, '/') & '/i/makeSElinks.cfm')>
						<form>
							<button id="btn_makeSearchEngineLinks" style="font-size: 10px; margin-top: 5px; margin-bottom: -20px;" onclick="openSElinks(); return false;">[Make Search Engine Links]</button>
						</form>
					</td>
				</cfif>	
			</cfoutput>
