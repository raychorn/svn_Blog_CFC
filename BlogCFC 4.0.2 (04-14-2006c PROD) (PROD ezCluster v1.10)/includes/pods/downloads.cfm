			...
			if (IsQuery(qDir)) {
				...
				for (i = 1; i lte qDir.recordCount; i = i + 1) {
					...
					if (session.persistData.loggedin) {
						...
						anchor = '(#_fSize# bytes)&nbsp;' & qDir.NAME[i] & '&nbsp;|&nbsp;' & _fStruct.descText;
					}
					formObj = '';
					...
					_hr = '';
					if (qDir.size[i] gt 0) writeOutput('<tr><td width="170" style="line-height: 14px;">(#i#)&nbsp;#formObj#&nbsp;<NOBR>#anchor#</NOBR>#_hr#</td></tr>');
				}
				writeOutput('</table>');
			}
		</cfscript>
