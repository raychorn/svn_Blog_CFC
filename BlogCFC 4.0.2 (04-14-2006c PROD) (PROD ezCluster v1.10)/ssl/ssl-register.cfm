...
<cfoutput>
	...
	<body>
		...
		<cfif (IsDefined("FORM.USERNAME")) AND (IsDefined("FORM.YOURNAME")) AND (IsDefined("FORM.PASSWORD")) AND (IsDefined("FORM.CONFIRMPASSWORD"))>
			...
			<cfset _urlValidateLink = Request.commonCode.clusterizeURL('http://rabid.contentopia.net') & ListSetAt(ListDeleteAt(ListSetAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), 'validateUserAccount/#FORM.USERNAME#/', '/'), 1, '/'), 1, 'blog', '/') & '?' & _uuid>
		...
		<cfset _url = Request.commonCode._clusterizeURLForSessionOnly(Request.commonCode.clusterizeURL('http://rabid.contentopia.net') & ListSetAt(CGI.SCRIPT_NAME, ListLen(CGI.SCRIPT_NAME, '/'), '_index.cfm', '/'))>
